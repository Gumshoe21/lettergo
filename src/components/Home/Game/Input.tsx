    ...
    if (englishWords.includes(inputRef.current?.value.toLowerCase() as string)) {
      ...
      for (let i = 0; i < correctWordLetters!.length; i++) {
        if (!randomLettersCopy.includes('A' || 'E' || 'I' || 'O' || 'U')) {
          randomLettersCopy = randomLettersCopy.join('').replace(`${correctWordLetters![i]}`, _.shuffle(['A', 'E', 'I', 'O', 'U']).filter(letter => !randomLettersCopy.includes(letter)).pop()).split('')
        } else {
          randomLettersCopy = randomLettersCopy.join('').replace(`${correctWordLetters![i]}`, _.shuffle(consonants).filter(letter => !randomLettersCopy.includes(letter)).pop()).split('')
        }
      }
    ...
    <>
      <form onSubmit={handleOnSubmit}>
        <div className="flex flex-row text-center justify-around gap-2 items-center py-4 px-4">
          <svg onClick={(e: React.MouseEvent<SVGElement>) => handleBackspace(e)} xmlns="http://www.w3.org/2000/svg" viewBox="0 0 20 20" fill="white" className="w-12 h-12 cursor-pointer">
