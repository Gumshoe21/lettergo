  ...
  const handleLetterClick = (e: any) => {
    if (!inputValue.includes(e.target.innerHTML.toUpperCase())) {
      ...
      setInputValue((inputValue) =>
        inputValue
          .split('')
          .filter((i) => i !== e.target.innerHTML.toUpperCase())
          .join('')
      )
    }
  }
  ...
  const handleOnSubmit = (e: any) => {
    ...
    for (let i = 0; i < correctWordLetters!.length; i++) {
      if (!randomLettersCopy.includes('A' || 'E' || 'I' || 'O' || 'U')) {
        randomLettersCopy = randomLettersCopy
          .join('')
          .replace(
            `${correctWordLetters![i]}`,
            _.shuffle(vowels)
              .filter((letter) => !randomLettersCopy.includes(letter))
              .pop() as string
          )
          .split('')
      } else {
    ...
    <>
      <form onSubmit={handleOnSubmit}>
        <div className='flex flex-row text-center justify-around gap-2 items-center py-4 px-4'>
          <svg
            onClick={(e: React.MouseEvent<SVGSVGElement>) => handleBackspace(e)}
          ...
          <Button disabled={!isActiveState} type={'submit'}>
            Enter
          </Button>
        </div>
      </form>
